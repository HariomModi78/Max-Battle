...
app.get("/tournament/payment/:userId/:tournamentId/:slotNumber",async function(req,res){
    ...
    if(tournament.matchType == "solo"){
        slot = await soloDataBase.findOne({tournamentId:tournament._id,slotNumber: req.params.slotNumber});
    }else if(tournament.matchType == "duo"){
        slot = await duoDataBase.findOne({tournamentId:tournament._id,slotId: req.params.slotNumber});
    }else if(tournament.matchType == "squad"){
        slot = await squadDataBase.findOne({tournamentId:tournament._id,slotId: req.params.slotNumber});
    }
    if(slot){
    ...
    let alreadyJoined;
    if(tournament.matchType == "solo"){
        alreadyJoined = await soloDataBase.findOne({tournamentId:tournament._id,userId:user._id});
    }else if(tournament.matchType == "duo"){
        alreadyJoined = await duoDataBase.findOne({tournamentId:tournament._id,userId:user._id});
    }else if(tournament.matchType == "squad"){
        alreadyJoined = await squadDataBase.findOne({tournamentId:tournament._id,userId:user._id});
    } 
    if (alreadyJoined) {
         return res.redirect("/");
    }
...
app.get("/tournament/detail/:userId/:tournamentId",async function(req,res){
    ...
    let joined;
    if(tournament.matchType == "solo"){
        joined = await soloDataBase.findOne({tournamentId:tournament._id,userId: req.params.userId});
    }else if(tournament.matchType == "duo"){
        joined = await duoDataBase.findOne({tournamentId:tournament._id,userId: req.params.userId});
    }else if(tournament.matchType == "squad"){
        joined = await squadDataBase.findOne({tournamentId:tournament._id,userId: req.params.userId});
    }
...
app.get("/adminPrizeDistribute/:adminId/:tournamentId",async function(req,res){
    ...
    if(tournament.matchType == "solo"){
        ...
        user = await duoDataBase.find({tournamentId:tournament._id});
    }else if(tournament.matchType == "squad"){
        user = await squadDataBase.find({tournamentId:tournament._id});
    } 
