...
const luckyDrawDataBase = require("./models/luckyDrawModel.js");
...
const {luckyDraw,adminCreateLuckyDraw,luckyDrawPayment,adminCreateLuckyDrawGet} = require("./routes/luckyDraw.js");

app.use("/",luckyDraw);
app.use("/",adminCreateLuckyDraw);
app.use("/",luckyDrawPayment);
app.use("/",adminCreateLuckyDrawGet);
...
app.get("/home/:userId",async function(req,res){
    try{
        ...
        //.log(tournament)
        let luckyDraw = await luckyDrawDataBase.find({status:"ongoing"});
        res.render("home",{user:user,notification:notification.length,adFlag:adFlag,tournament:tournament,luckyDraw});
    }catch(e){
        res.redirect("/error");
    }
...
app.get("/tournament/payment/:userId/:tournamentId/:slotNumber",async function(req,res){
    ...
    if (alreadyJoined) {
        ...
        res.render("tournamentPayment",{user:user,tournament:tournament,slotNumber:slotNumber,luckyDrawFlag:false});
        }catch(e){
        res.redirect("/error");
    }
})
...
app.post("/roomIdAndPassword/:userId/:tournamentId/:slotNumber",async function(req,res){
    ...
    const updatedTournament = await tournamentDataBase.findOneAndUpdate({_id:req.params.tournamentId},{
        ...
        if(Number(tournament.slotsFilled)+1 == Number((tournament.totalSlots)/2)){
  ...
  <a href="max-battle.onrender.com/tournament/${tournament.status}/${tournament.modeType}/${tournament.matchType}/${tournament.entryFee==0?"free":(tournament.entryFee==1)?"1rs":"paid"}/${users[i]._id}" style="padding: 10px 20px; background-color: #ff6f00; color: white; text-decoration: none; border-radius: 5px; font-weight: bold; box-shadow: 0 2px 6px rgba(0,0,0,0.2);">
    🔥 Join Now
  </a>
</div>
`
        ...
        if(Number(tournament.slotsFilled)+1 == Number((tournament.totalSlots)-1)){
            let users = await userDataBase.find({emailPermission:true});
            //.log(users.length);
            //.log("Han yahe hai users")
