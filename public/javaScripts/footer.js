...
for(let i=0;i<footerBox.length;i++){
    footerBox[i].addEventListener("click", function() {
        ...
        }else if(footerBox[i].classList[1]=="notification"){
            localStorage.setItem("wallet",1)
            mainHeader.style.display = "none";
            if(document.querySelector(".notificationNumber")){
               document.querySelector(".notificationNumber").style.cssText = "display:none" 
            }
            let header = document.createElement("div");
            header.classList = "header";
            let back = document.createElement("div");
